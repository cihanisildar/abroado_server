...
class GoogleOAuthService {
  ...
  public async processGoogleProfile(profile: Profile): Promise<GoogleAuthResult> {
    try {
      ...
      let existingUser = await userRepository.findByGoogleId(this.prisma, googleId);
      
      if (existingUser) {
        console.log(`[OAuth] Found existing user by Google ID: ${email}`);
        return {
          user: this.transformToUserResponse(existingUser),
          isNewUser: false
        };
      }

      // Try to find existing user by email
      existingUser = await userRepository.findByEmail(this.prisma, email);
      ...
      if (existingUser) {
        ...
        return {
          user: this.transformToUserResponse(updatedUser),
          isNewUser: false
        };
      }
      ...
      const newUser = await userRepository.create(this.prisma, userData);

      console.log(`[OAuth] Created new user successfully: ${email}`);

      return {
        user: this.transformToUserResponse(newUser),
        isNewUser: true
      };
