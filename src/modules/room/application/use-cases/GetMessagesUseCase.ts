...
export class GetMessagesUseCase {
    ...
    async execute(roomId: string, userId: string, query: any): Promise<{ messages: Message[]; total: number; pagination: any }> {
        if (!userId) {
        ...
        return await this.messageRepository.findByRoomId(roomId, query);
    }
}
